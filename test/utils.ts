...
export const seedTrackAudio = async (trackId: number, extension = "wav") => {
  const audio = await prisma.trackAudio.findFirstOrThrow({
    where: { trackId },
  });
  const updated = await prisma.trackAudio.update({
    where: { id: audio.id },
    data: { fileExtension: extension },
  });
  await uploadWrapper(
    finalAudioBucket,
    `${audio.id}/original.${extension}`,
    Readable.from(createSilentWavBuffer())
  );
  return updated;
};

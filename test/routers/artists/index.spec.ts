...
describe("artists", () => {
  ...
  describe("/", () => {
    ...
    it("should GET / with 1 artist in the database", async () => {
      ...
      const trackGroup = await createTrackGroup(artistProfile.id);
  ...
  describe("RSS", () => {
    it("should GET / in RSS format", async () => {
      ...
      assert.equal(obj.items[0].title, `${artistProfile.name}`);
    });
  });
  ...
  describe("Label filtering", () => {
    it("should return all artists (labels and non-labels) when isLabel is not specified", async () => {
      ...
      const artistProfile = await createProfile(user1.id, {
        name: "Regular Artist",

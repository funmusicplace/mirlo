...
describe("artists", () => {
  ...
  describe("/{id}", () => {
    it("should GET /{id} with artist slug", async () => {
      ...
      const artistProfile = await prisma.profile.create({
        data: {
          name: "Test artist",
          urlSlug: profileSlug,
          userId: user.id,
      ...
      assert.equal(response.body.result.id, artistProfile.id);
    });
    ...
    it("should return the artist's currency on trackGroups, not 'usd'", async () => {
      ...
      await createTrackGroup(artistProfile.id, { title: "Euro Album" });

      const response = await requestApp
        .get(`artists/${artistProfile.urlSlug}`)
        .set("Accept", "application/json");
    ...
    it("should return trackGroup tracks ordered by 'order' asc", async () => {
      ...
      await createTrackGroup(artistProfile.id, {
        title: "Ordered Album",

      ...
      csvData.forEach((row, rowIndex) => {
        ...
        if (!grouped[key]) {
          grouped[key] = {
            ...
            tracks: [],
            minPrice:
              Number(getMappedValue(row, "min_price", mapping, headers)) ||
              undefined,
            releaseDate: getMappedDateValue(
              row,
              "release_date",
              mapping,
              headers
            ).toISOString(),
            publishedAt: getMappedDateValue(
              row,
              "published_date",
              mapping,
              headers
...
function getMappedDateValue(
  ...
  if (columnIndex === undefined) return new Date();
  const header = headers[parseInt(columnIndex)];
  return (row[header] as unknown as Date) || new Date();
}

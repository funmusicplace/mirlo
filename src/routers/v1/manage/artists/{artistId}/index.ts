...
export default function () {
  ...
  async function PUT(req: Request, res: Response, next: NextFunction) {
    const { artistId: artistProfileId } = req.params as unknown as Params;
    const {
      bio,
      name,
      urlSlug,
    ...
    try {
      ...
      if (updatedCount) {
        const artistProfile = await prisma.profile.findFirst({
          where: { id: Number(artistProfileId) },
        });
        res.json({
          result: artistProfile
            ? processSingleProfile(artistProfile, Number(user.id))
            : artistProfile,
        });
      } else {
        res.json({
  ...
  async function GET(req: Request, res: Response, next: NextFunction) {
    ...
    const castProfileId = await findProfileIdForURLSlug(artistProfileId);
    try {
      const artistProfile = await prisma.profile.findFirst({
        where: {
      ...
      if (!artistProfile) {
        ...
        return res.json({
          result: processSingleProfile(artistProfile, Number(user.id)),
        });
      }
    } catch (e) {
      next(e);
  ...
  async function DELETE(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      await deleteProfile(Number(user.id), Number(artistProfileId));
    } catch (e) {
      return next(e);
    }

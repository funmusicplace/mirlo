  ...
  async function GET(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const results = subscribers.map((sub) => {
        ...
        const { profileId: tierProfileId, profile, ...tierRest } =
          profileSubscriptionTier;
        return {
          ...subRest,
          artistSubscriptionTierId: profileSubscriptionTier.id,
      ...
      if (req.query?.format === "csv") {
        return downloadCSVFile(res, "subscribers.csv", csvColumns, results);
      } else {
        res.json({ results });
      }
    } catch (e) {
